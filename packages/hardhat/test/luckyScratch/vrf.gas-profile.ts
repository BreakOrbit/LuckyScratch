...
describe("VRF fulfillPoolRandomness gas profiling", () => {
  const ticketCounts = [10, 50, 100, 200];

  for (const totalTickets of ticketCounts) {
    it(`measures gas for ${totalTickets} tickets`, async () => {
      ...
      const tx = await deployed.vrfAdapter.connect(deployed.admin).fulfillRandomness(round.vrfRequestRef, 777n);
      const receipt = await tx.wait();

      console.log(`\n  Tickets: ${totalTickets} | Gas used: ${receipt!.gasUsed.toString()}`);

      const stateAfter = await deployed.core.poolStates(POOL_ID);
      const roundAfter = await deployed.core.roundStates(POOL_ID, stateAfter.currentRound);
      expect(stateAfter.vrfPending).to.equal(false);
      expect(roundAfter.status).to.equal(1); // PendingEncryption
    });
  }
});
