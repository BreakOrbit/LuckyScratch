...
/**
 * Compact single-row pool info panel:
 *   Row 1: [Cover] [Name + subtitle] ——— [PRICE] [WIN RATE] [MAX PRIZE]
 *   Row 2: Remaining | Win Type | Pool Total | Issuer | Description
  ...
  return (
    <div className="relative mb-6 rounded-2xl overflow-hidden">
      ...
      <div className="relative z-10 px-5 md:px-7 py-4 md:py-5">
        ...
        {/* ═══ Row 1: Cover + Name + Stats (single row) ═══ */}
        <div className="flex items-center gap-4 md:gap-5 mb-3">
          {/* Pool Cover */}
          <div
            className="flex-shrink-0 w-[52px] h-[52px] md:w-[60px] md:h-[60px] rounded-xl overflow-hidden border border-white/10"
            style={{
              boxShadow: "0 0 16px rgba(198,40,40,0.12)",
            }}
          >
            {coverImage ? (
              <img src={coverImage} alt={poolName} className="w-full h-full object-cover" />
            ) : (
              <div
              ...
              >
                ...
                />
                <span className="text-2xl md:text-3xl relative z-10 drop-shadow-lg">{poolEmoji}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-end gap-6 md:gap-10 flex-shrink-0">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-5 gap-2">
            {prizes.map((prize, i) => (
              ...
              >
                ...
                <div>
                  <div className="font-headline font-bold text-sm text-white leading-tight">{prize.amount}U</div>
