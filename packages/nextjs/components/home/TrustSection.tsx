...
const pillars: TrustPillar[] = [
  ...
  },
  {
    ...
    description: "Every contract, Every pool, Every payout is logged eternally on the ledger for total transparency.",
    accentColor: "primary",
  },
];
        ...
        {pillars.map(pillar => {
          ...
          return (
            <div key={pillar.title} className="flex flex-col items-center text-center group">
              ...
              >
                <span className={`material-symbols-outlined ${style.text} text-3xl`}>{pillar.icon}</span>
              </div>
              <h4 className="text-xl font-headline font-bold text-ns-on-surface mb-3 uppercase tracking-wider">
                {pillar.title}
              </h4>
              <p className="text-sm text-ns-on-surface-variant leading-relaxed">{pillar.description}</p>
            </div>
          );
        })}
      </div>
