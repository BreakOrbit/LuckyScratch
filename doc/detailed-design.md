...
- **B 组标签：** `💥 搏大奖` `高倍回报` `刺激挑战`
- **价格标签：** `2U` `5U` `10U` `15U`
- **池实例数量：** 实现中保留为独立 `pool instance` 数量，每个实例独立运行
...
- 不在最后一笔购票交易里自动完成下一轮刷新
- 采用显式链上刷新入口触发下一轮
- 后端在检测到最后一张票售出后，自动触发一次链上刷新入口
- 若该次自动触发失败，再进入重试与兜底调度；最终是否刷新仍由合约状态机决定
...
- VRF 请求与回调成本按 pool 维度计提。
- 当前文档中的“池实例数量”在实现里保留为独立 `pool instance`。
...
| 数据 | 加密方式 | 说明 |
| --- | --- | --- |
| 奖池余额 | `euint64` | 每个池实例当前剩余的总奖金 |
| 奖项库存 | `euint32[]` | 每个池实例各档位的剩余数量 |
| 剩余票数 | `euint32` | 每个池实例剩余可购票数 |
...
5. 用户发起刮奖后，先调用链上 `scratchTicket()`，由合约校验当前持有人并将 NFT 状态更新为"已刮开"。
6. 链上状态更新成功后，前端再对该 NFT 绑定的加密结果发起授权解密并本地展示。
7. 当前 NFT 持有人可查看结果，并在中奖时发起领奖。
...
| 已领奖 | ❌ 不可转让 | — | 奖励已记入用户钱包中的加密 cUSDC 余额 |
...
- 用户刮开时，不是重新抽奖，而是先通过链上 `scratchTicket()` 锁定状态，再对该 NFT 绑定结果进行授权解密与展示。
...
```mermaid
graph LR
    A[用户钱包中的官方 cUSDC] -->|购票| B[奖池 / 奖票系统]
    B -->|中奖后领奖| C[用户加密 cUSDC 余额]
    B -->|利润分配| D[Buffer / Platform]
```
...
- 领奖入口：刮开后 claim 到用户加密 cUSDC 余额

解密授权说明：

- Zama fhEVM 的具体解密授权接口由后端服务实现。
- 前端不直接持有解密授权逻辑，只负责发起请求和展示结果。
...
| 奖项分布 | 创建者自行分配各奖项金额和数量 | 所有奖项金额之和 = 奖金总额 比如5U*10张，10U*5张，50U*1张|
| 循环模式 | 一次性 / 循环 | 详见 8.4 |
| 池主题色 | 上传池子封面和彩票封面| 影响池卡片和彩票视觉 |
...
│  购买数量:  [-] 3 [+]  (1-10 张可选)        │
│  总价: 6 USDC                             │
下面是一个带空间感背景的页面，上面有一排彩票卡片排列，
支付完成后会有一个类似洗牌的那种动画，然后显示购买成功几张
│                                          │
│  【自选模式】                              │
│  不要网格，就是一排到两排彩票卡片排列 ，
立体感，立体背景，有换一批按钮，点击换一批                  │
│  已选: 3 张 · 总价: 6 USDC                 │
├──────────────────────────────────────────┤
│  钱包状态检查                              │
...
graph TD
    A[首页 /] --> B[彩票商店 /store]
    A --> C[官方池购票页 /store/official/[poolType]]
    A --> D[创建池子 /create-pool]
    A --> E[我的彩票 /my-tickets]
    A --> F[个人中心 /profile]

    B --> C
    B --> G[用户池购票页 /store/community/[poolId]]

    C --> H[官方池自选页 /store/official/[poolType]/select]
    G --> I[用户池自选页 /store/community/[poolId]/select]

    C --> J[购买成功弹窗]
    G --> J
    J --> K[单张刮奖页 /scratch/[tokenId]]
    J --> E

    E --> K
    E --> L[批量刮奖结果页 /scratch/batch]
    E --> M[领奖弹窗]
    E --> N[转让弹窗]

    F --> O[我的池子 /my-pools]
    O --> P[池子运营详情页 /my-pools/[poolId]]
```
...
| 页面 | 推荐路由 | 主要用途 | 核心功能 |
| --- | --- | --- | --- |
| 首页 | `/` | 品牌入口、活动承接、精选池转化 | 顶部广播、Hero、平台统计、官方池精选卡片、创建池入口、快捷跳转 |
| 彩票商店 | `/store` | 承接完整池浏览，不让首页承担全部列表压力 | 官方池 / 用户池 Tab、筛选、排序、搜索、池卡片列表、进入详情 |
| 官方池购票页 | `/store/official/[poolType]` | 承接平台 8 个标准主题池的详情与购买 | 池主题海报、奖项结构、机选购买、余额检查、购买成功弹窗 |
| 官方池自选页 | `/store/official/[poolType]/select` | 为平台标准卡面提供规则统一的自选入口 | 标准编号网格、已售/可选状态、数量限制、价格汇总、确认购买 |
| 用户池购票页 | `/store/community/[poolId]` | 展示用户自建池详情并承接购买 | 创建者信息、票价、中奖率、最高奖金、销售进度、模式标签、机选购买 |
| 用户池自选页 | `/store/community/[poolId]/select` | 兼容用户自定义票面风格与布局的自选入口 | 非固定卡面布局、票面主题预览、已售占位、可选卡片、多选确认 |
| 我的彩票 | `/my-tickets` | 持票资产中心，承接刮奖、领奖、转让 | 状态筛选、来源筛选、总中奖金额统计、未领奖汇总、单张刮奖、批量刮奖、转让、领奖 |
| 单张刮奖页 | `/scratch/[tokenId]` | 沉浸式单票刮奖体验 | 刮涂层动画、70% 触发链上刮奖、解密过渡、单票结果展示、去领奖 / 下一张 |
| 批量刮奖结果页 | `/scratch/batch` | 多票连续开刮后的汇总结果承接页 | 总张数、中奖张数、总金额、逐票结果、一键领奖、返回我的彩票 |
| 个人中心 | `/profile` | 用户账户、统计、历史流水总览 | 钱包信息、购票数、中奖率、中奖金额、历史记录、个人收益概览 |

### 14.10.2 创建者端主页面

| 页面 | 推荐路由 | 主要用途 | 核心功能 |
| --- | --- | --- | --- |
| 创建池子页 | `/create-pool` | 完成自建池发行配置与保证金锁定 | 基本信息、奖金结构、中奖率、RTP、循环模式、费用预估、创建确认 |
| 我的池子 | `/my-pools` | 创建者查看自己所有池子的经营状态 | 池状态、销售进度、轮次、销售收入、手续费、已派奖金额、可提利润、保证金状态 |
| 池子运营详情页 | `/my-pools/[poolId]` | 深入查看单个池子的完整经营与资金信息 | 轮次明细、售票趋势、利润曲线、关闭池子、提取利润、退还保证金、查看规则 |

### 14.10.3 关键弹层与辅助界面

| 界面 | 触发位置 | 主要用途 | 核心功能 |
| --- | --- | --- | --- |
| 购买成功弹窗 | 官方池 / 用户池购票成功后 | 承接购买后的下一步分流 | 立即刮奖、存入我的彩票、继续购买 |
| 领奖弹窗 | 我的彩票、单张刮奖页、批量刮奖页 | 在中奖后执行单张或批量领奖 | 展示待领金额、校验 proof、确认领奖、成功反馈 |
| 转让弹窗 | 我的彩票未刮开卡片 | 承接 NFT 转让动作 | 输入地址、风险提示、确认转让 |
| 钱包连接弹窗 | 顶部导航、未连接状态按钮 | 连接钱包以完成购票和刮奖 | RainbowKit 连接、切换账户、切换网络 |
| 售罄/初始化中/关闭态空界面 | 池详情、商店卡片、我的池子 | 处理特殊状态，不额外开新页面 | 显示不可购买原因、等待刷新、返回商店、关注后续轮次 |

## 14.11 基于文档补充后的页面边界说明

### 14.11.1 你当前总结里需要修正的点

- 平台默认池数量按当前文档不是 10 个，而是 **8 个官方主题池**，分为 A 组和 B 组。
- 首页不应承载所有池的完整浏览，**还需要一个独立的彩票商店页** 来展示完整的官方池与用户池。
- 购票页至少应拆成 **官方池购票页** 和 **用户池购票页** 两类，二者的信息结构不同。
- 自选页也应拆成两类：**官方池标准编号网格页** 与 **用户池多样卡面自选页**。

### 14.11.2 根据文档新增补出的页面

- 彩票商店页：你当前总结里缺失，但顶部导航与用户池展示逻辑都需要它。
- 用户池购票页：用户自建池需要单独详情承接，不适合完全复用官方池详情页。
- 我的池子页：创建者端需要查看自己所有池子的经营状态。
- 池子运营详情页：文档里已有“查看详情”动作，因此需要二级详情页，不应只停留在列表层。

### 14.11.3 不必独立成页面、但必须独立设计的界面
...
- 一期文档中“了解更多”“规则说明”“中奖广播”更适合作为首页区块或弹层，不强制要求独立页面。
- 若后续需要 SEO 或内容运营，再拆出 About / Rules / FAQ 独立页面。
...
| 最大购买量 | 10 张（单次） |
| 池售罄 | 不可购买，按钮灰色 + "售罄" |
| cUSDC 余额不足 | 按钮灰色 + "余额不足" |
| 重复购买 | 允许，每次都铸造新 NFT |
| cUSDC 授权 | 首次使用时需要完成官方 cUSDC 合约授权 |
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/` | 首页 | 品牌入口 + 精选官方池 + 广播 + 创建池子入口 |
| `/store` | 彩票商店 | 完整池列表，支持官方池 / 用户池切换、排序、筛选 |
| `/store/official/[poolType]` | 官方池购票页 | 平台 8 个标准主题池的详情与购票页 |
| `/store/official/[poolType]/select` | 官方池自选页 | 标准编号网格自选 |
| `/store/community/[poolId]` | 用户池购票页 | 用户自建池详情与购票页 |
| `/store/community/[poolId]/select` | 用户池自选页 | 兼容多样卡面样式的自选页 |
...
| `/my-pools/[poolId]` | 池子运营详情页 | 单个池子的轮次、利润、关闭与退款操作 |
| `/profile` | 个人中心 | 钱包/历史/统计 |
...
基于本详细设计文档，总结 LuckyScratch 项目所需的完整前端页面结构（路由树）及页面内构成模块梳理如下。

## 1. 全局公共组件
- **TopNav (顶部导航栏)**
  - 左侧：Logo (🎰 LuckyScratch)
  - 中间导航：[首页] [彩票商店] [我的彩票] [创建池子]
  - 右侧：钱包连接状态 (👤 地址与余额 / 连接钱包按钮)
- **Footer (页脚)**：版权信息、快速链接、合约地址
- **Wallet Modal**：RainbowKit 钱包连接弹窗
- **Toast/Notification**：全局游戏交互提示与错误提醒 (使用 notification / getParsedError)

## 2. 页面层级与模块拆解

### 2.1 首页 (Home Page)
- **路由路径**：`/`
- **页面职责**：承担品牌入口、活动承接和高转化分流，不承担完整池浏览
- **页面结构**：
  - **动态滚动组件**：`中奖广播滚动条`
  - **主视觉区 (Hero Banner)**：大标题、副标题与 CTA
  - **平台实时统计栏**：总奖金池、已发奖金、活跃用户等
  - **官方池精选区**：默认展示平台官方 8 个主题池的精选卡片
  - **快捷入口区**：`[进入彩票商店]` `[创建我的池子]`
  - **平台价值展示区**：FHE 加密、不可博弈、公正透明

### 2.2 彩票商店与购票链路
- **2.2.1 彩票商店页 (Lottery Store)**
  - **路由路径**：`/store`
  - **页面职责**：展示完整的官方池与用户池，提供筛选、排序、搜索与跳转
  - **页面结构**：
    - **顶部筛选区**：`[🏛️ 官方池]` `[👤 用户池]`
    - **排序器**：最新、热门、中奖率、票价
    - **搜索/筛选栏**：池名、价格区间、模式、状态
    - **池卡片列表**：按 Tab 渲染官方池或用户池

- **2.2.2 官方池购票页 (Official Purchase Page)**
  - **路由路径**：`/store/official/[poolType]`
  - **页面职责**：承接平台标准池详情与标准化购票流程
  - **页面结构**：
    - **池基本概览卡**：主题海报、票价、最高奖金、中奖率、组别
    - **奖项结构折叠区**
    - **购票方式切换**：`[🎲 机选]` 与 `[✋ 自选]`
    - **机选场景**：数量选择器、价格汇总、余额检查
    - **支付区**：`[确认购买]`
    - **结果引导弹层**：购买成功后进入立即刮奖或我的彩票

- **2.2.3 官方池自选页 (Official Select Page)**
  - **路由路径**：`/store/official/[poolType]/select`
  - **页面职责**：承载平台标准票面编号网格的自选体验
  - **页面结构**：
    - **票面编号网格 (N×M Grid)**
    - **售出/可选/选中状态反馈**
    - **底部确认栏**：已选数量、总价、确认购买

- **2.2.4 用户池购票页 (Community Purchase Page)**
  - **路由路径**：`/store/community/[poolId]`
  - **页面职责**：承接用户自建池详情与购买
  - **页面结构**：
    - **池信息头部**：池名称、创建者、模式、销售进度、最高奖金
    - **池规则区**：中奖率、RTP、票价、总票数、主题说明
    - **购票区**：机选数量、余额校验、确认购买
    - **可选入口**：进入兼容多样卡面的自选页

- **2.2.5 用户池自选页 (Community Select Page)**
  - **路由路径**：`/store/community/[poolId]/select`
  - **页面职责**：兼容不同创建者票面风格与布局，不强制使用标准编号网格
  - **页面结构**：
    - **票面卡片画廊 / 异形网格**
    - **已售占位与不可选态**
    - **多选确认栏**：选中张数、金额合计、确认购买

### 2.3 我的核心资产与彩票库
- **我的彩票 (My Tickets Page)**
  - **路由路径**：`/my-tickets`
  - **页面职责**：统一承接用户持票、刮奖、领奖、转让
  - **页面结构**：
    - **多维度过滤 Tab**：`[全部]`、`[未刮开]`、`[已刮开]`、`[有奖待领]`
    - **来源筛选**：`[平台彩票]`、`[彩票商店彩票]`
    - **统计区**：总票数、已中奖金额、待领取金额、中奖率
    - **群组操作栏**：批量开刮、一键全刮、全部领奖
    - **NFT 持票网格列表**：未刮、已刮、中奖、未中奖、可转让状态

### 2.4 加密刮奖环节与中奖响应系
- **2.4.1 单张深度刮奖页 (Scratch Focus)**
  - **路由路径**：`/scratch/[tokenId]`
  - **页面职责**：提供沉浸式单票刮奖与私密揭晓体验
  - **页面结构**：
    - **Canvas 刮擦主区域**
    - **解密过渡区**：`Decrypting result...`
    - **结果揭晓区**：无奖 / 小奖 / 大奖分层反馈
    - **操作区**：领奖、刮下一张、返回我的彩票

- **2.4.2 批量全域抽奖速报 (Batch Results)**
  - **路由路径**：`/scratch/batch`
  - **页面职责**：承接批量开刮后的结果汇总与批量领奖
  - **页面结构**：
    - **总览顶栏**：张数、中奖张数、总金额
    - **逐票结果卡**
    - **聚合领奖区**
...
### 2.5 用户代理自建池发行业务模块
- **2.5.1 池发行参数编辑器 (Create Pool)**
  - **路由路径**：`/create-pool`
  - **页面职责**：完成建池配置、规则校验和保证金确认
  - **页面结构**：
    - **基础信息区**
    - **资金与奖项配置区**
    - **中奖率 / RTP / 利润预估区**
...
- **2.5.2 我主理的池中心 (My Pools Panel)**
  - **路由路径**：`/my-pools`
  - **页面职责**：查看自己名下所有池子的经营状态
  - **页面结构**：
    - **创建者总览统计**
    - **池列表卡片**
    - **快捷操作**：提取利润、关闭池子、查看详情

- **2.5.3 池子运营详情页 (Pool Operations Detail)**
  - **路由路径**：`/my-pools/[poolId]`
  - **页面职责**：查看单池轮次、资金和风险状态
  - **页面结构**：
    - **池头部概览**
    - **轮次与销售趋势区**
