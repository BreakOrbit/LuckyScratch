...
- The profile frontend sidebar `My Pools` action now renders the converted right-side card grid layout from `doc/profile/stitch_ (4)/code.html` via `packages/nextjs/components/profile/MyPoolsPanel.tsx`; the pool cards no longer show a refund-bond CTA, and `Withdraw` now opens a profit breakdown modal with the calculation formula and amount line items
