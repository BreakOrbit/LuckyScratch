...
- Production deployment now targets real Zama confidential `cUSDC` wrapper addresses and real Chainlink VRF v2.5 network settings on supported networks such as Sepolia; no local mock token is deployed by the LuckyScratch deploy script, and every fresh live redeploy requires adding the new `LuckyScratchVRFAdapter` address as a consumer on the configured Chainlink VRF subscription before `createPool` can request randomness
...
- Pool cover frames now use a canonical horizontal 16:9 frame across full pool-cover surfaces including store cards, homepage pool cards, purchase headers, pool detail headers, pool ranking podium cards, creator pool cards, and the create-pool cover upload preview; ticket artwork and small identity thumbnails remain separate surfaces
- Ticket artwork displays now use a canonical vertical 9:16 frame across ticket-art previews, purchase selection/success ticket thumbnails, scratch views, and wallet ticket inventory; the My Tickets vault shows the pool `ticketArtUrl` for scratched/claimed tickets while keeping all ticket cards on that page in the same vertical media ratio
...
- The purchase frontend now renders a live ticket-selection and checkout experience under `packages/nextjs/app/purchase/[poolId]/page.tsx` with the main body in `packages/nextjs/components/purchase/PurchasePage.tsx`; it reads backend purchase-context data, loads pool prize tiers from backend metadata or the pool metadata gateway JSON for the Prize Structure panel, blocks checkout until the indexed pool is active and the current round is `Ready`, derives sold/available ticket indexes from the indexed round, checks whether `LuckyScratchTreasury` is a cUSDC operator, submits `setOperator` when needed, executes `LuckyScratchCore.purchaseTickets` / `purchaseTicketsWithSelection`, parses `TicketPurchased`, and routes minted ticket ids into the scratch flow
- The scratch frontend now renders the scratch experience under `packages/nextjs/app/scratch/[poolId]/page.tsx` with the main body in `packages/nextjs/components/scratch/ScratchPage.tsx`; a single `tickets` query id renders `SingleScratchView`, multiple ids render `BatchScratchView`, and the views submit wallet-driven `LuckyScratchCore.scratchTicket` / `batchScratch`, then immediately request backend reveal-auth and Zama `publicDecrypt` for the scratched ticket results before showing the post-scratch animation, with My Tickets remaining as the retry / claim surface instead of exposing the old reveal workspace route
- The create-pool frontend under `packages/nextjs/app/create-pool/page.tsx` / `packages/nextjs/components/create-pool/CreatePoolPage.tsx` now uses the terminal-style pool creation layout, derives Lottery Win Type from total tickets plus reward tier amounts instead of direct user selection, supports community vs official pool type selection with frontend `ADMIN_ROLE` verification before official-pool submission, requires live `cUSDC` / treasury metadata before uploading backend draft assets, checks and submits confidential cUSDC operator authorization through `packages/nextjs/contracts/externalContracts.ts`, validates selected artwork against the default 10 MB backend upload limit, uploads cover/ticket artwork to the backend IPFS service, creates a backend metadata draft, calls `LuckyScratchCore.createPool`, parses `PoolCreated`, and finalizes pool metadata binding back through the backend
- `packages/nextjs/app/faucet/page.tsx` now provides a Sepolia cUSDC faucet that mints Zama's public mock underlying USDC, approves and wraps it into confidential `cUSDCMock`, can user-decrypt the wallet's confidential cUSDC balance after an EIP-712 signature, and can authorize `LuckyScratchTreasury` as the wallet's confidential cUSDC operator
- The frontend public env template now lives in `packages/nextjs/.env.example`; `NEXT_PUBLIC_BACKEND_URL` controls backend API requests, `NEXT_PUBLIC_IPFS_GATEWAY_BASE_URL` can rewrite backend-returned Pinata/IPFS asset URLs for display, `NEXT_PUBLIC_SEPOLIA_RPC_URL` overrides the Sepolia RPC used by wagmi plus the browser-side Zama relayer SDK, and `NEXT_PUBLIC_ZAMA_RELAYER_URL` can override the browser-side Sepolia Zama relayer URL when a deployment needs a reachable relayer/proxy
...
- `packages/nextjs/app/my-tickets/page.tsx` now renders a dedicated wallet-driven ticket vault through `packages/nextjs/components/my-tickets/MyTicketsVault.tsx`; the vault reads paginated/filterable backend ticket inventory by status and pool id, can open same-pool selections into the scratch queue, and lets scratched reveal-authorized tickets request backend reveal-auth, obtain Zama `publicDecrypt` KMS proofs through ticket-scoped backend proxies, and submit either `LuckyScratchCore.claimReward` or selected-ticket `batchClaimRewards` directly from the wallet; the old `packages/nextjs/app/tickets/` reveal workspace and scaffold `packages/nextjs/app/debug/` page have been removed
...
- The backend now includes an IPFS upload client (`packages/backend/ipfs/`), pool metadata draft/finalize service (`packages/backend/poolmeta/`), SQL tables for uploaded assets and pool metadata drafts, and API routes for image upload, metadata draft creation, pool finalize, creator pool summaries, creator-filtered pool lists, purchase-context reads, enriched pool/current-round reads, paginated/filterable wallet ticket reads, platform overview stats, recent claimed wins, and player leaderboards
- The backend migration now includes the LuckyScratch read model, `deployment_registry`, `indexed_logs`, recurring `jobs`, audit/cost tables, pool metadata tables, and read-model query indexes for creator pool lists, pool ticket lists, wallet ticket filters, recent wins, player leaderboards, and overview status counts needed by the live API/worker
...
- Backend go-ethereum contract bindings decode Solidity public struct mapping getters such as `poolConfigs`, `poolStates`, `poolAccounting`, `roundStates`, and `tickets` from their flat ABI output fields instead of treating them as a single tuple, so API finalize and worker indexing do not panic on `PoolConfig` reads
...
- The frontend now loads the browser-side relayer SDK from the official UMD script on demand instead of before every page becomes interactive, keeps ticket decryption keypairs in an in-memory runtime provider, and initializes claim/decryption flows against the backend-issued ticket-scoped relayer URL rather than a direct upstream relayer URL
- The backend still exposes ticket detail, reveal-auth, claim-precheck, and Zama proxy APIs, but the standalone frontend reveal workspace route has been removed from the current navigation surface
...
- The current My Tickets claim flow uses backend `reveal-auth` to authorize ticket-scoped Zama proxies, calls SDK `publicDecrypt` for scratched ticket handles, rejects zero-reward decrypts client-side, and then submits `claimReward` for one winning ticket or `batchClaimRewards` for selected winning tickets with the returned clear amounts and proofs
...
- `NEXT_PUBLIC_ZAMA_RELAYER_URL` is optional and overrides the browser-side Sepolia Zama relayer URL used by faucet balance decryption and direct relayer SDK flows; when unset, the frontend uses Zama's official Sepolia v2 relayer URL
...
- GitHub Actions now also runs `go test ./...` in `packages/backend`, `yarn hardhat:test`, and `yarn next:build` in addition to lint and type checks, so full-stack changes should keep those passing locally when practical
...
- The LuckyScratch deploy script expects a real `cUSDC` address and a funded Chainlink VRF v2.5 subscription id for Sepolia / mainnet-style deployments; it performs a full fresh redeploy by default unless `LUCKY_SCRATCH_REUSE_EXISTING=true` is set, prints the `LuckyScratchVRFAdapter` consumer address that must be added to the subscription, and local tests deploy the VRF adapter in mock mode by passing a zero coordinator
